...
const Profile = () => {
  ...
  const [historyData] = useState([
    ...
    { id: 2, title: "Elektrik xəttinin çəkilməsi", startDate: "26 Aprel", endDate: "29 Aprel", status: "İcardadır" },
    { id: 3, title: "Kondisioner quraşdırılması", startDate: "24 Aprel", endDate: "24 Aprel", status: "Tamamlandı" },
  ]);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: user.fullname,
          text: `${user.fullname} profilinə baxın!`,
          url: window.location.href,
        });
      } catch (err) { console.log("Paylaşım uğursuz oldu"); }
    } else {
  ...
  // Zəng et funksiyası
  const handleCall = () => {
    const phone = user.phone || "+994500000000"; // API-dan nömrə gəlmədikdə default
    window.location.href = `tel:${phone}`;
  };

  // WhatsApp funksiyası
  const handleWhatsApp = () => {
    const phone = user.phone ? user.phone.replace(/\D/g, '') : "994500000000";
        ...
        <div className="absolute top-6 left-4 right-4 max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-md rounded-full text-white border border-white/20 shadow-lg hover:bg-white/30 transition-all"
            >
